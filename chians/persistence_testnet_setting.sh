...
# Initialize Oracle-feeder
echo "Installing $ORACLE_NAME..."
cd $HOME
cd ~/go/src/github.com
git clone $ORACLE_GITURL
cd $ORACLE_NAME
git checkout $ORACLE_BRANCH
make install
...
# Oracle-feeder service setup
sudo tee <<EOF >/dev/null /etc/systemd/system/$ORACLE_BINARY.service
[Unit]
...
[Service]
Environment="ORACLE_FEEDER_KEY_PASSPHRASE=$ORACLE_FEEDER_KEY_PASSPHRASE"
Type=simple
User=root
